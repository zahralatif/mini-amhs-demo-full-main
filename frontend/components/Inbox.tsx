...
export default function Inbox({ refreshKey }: { refreshKey: number }) {
  ...
  const columns: GridColDef[] = useMemo(() => [
    ...
    { field: 'created_at', headerName: 'Time', width: 200, valueGetter: (v) => new Date(v?.value as string).toLocaleString() },
  ], []);
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        setSnack({ open: true, message: e.message || 'Failed to load inbox', severity: 'error' });
      } finally {
        setLoading(false);
      }
    }
